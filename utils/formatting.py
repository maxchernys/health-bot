...
def _has_data(d: dict) -> bool:
    """Check if a device dict has any non-None values (ignoring _raw keys)."""
    return any(v is not None for k, v in d.items() if not k.startswith("_"))
...
def format_health_summary(data: dict) -> str:
    ...
    has_whoop = _has_data(w)
    has_oura = _has_data(o)
    ...
    if has_whoop and w_strain and w_strain >= 1.0:
        lines.append(f"\n*Whoop — Workout*")
